...
public class MainActivity extends AppCompatActivity {
    ...
    private void computeButtonStart(View view){
        String textNameX = getInputOfTextField(R.id.inputNameX);
        String textNameO = getInputOfTextField(R.id.inputNameO);
        String textPlaysFirst = getItemSelected(R.id.optionPlaysFirst);

        char playsFirst = textPlaysFirst.charAt(0);

        Game x = new Game(textNameX, textNameO);
        x.setWhoPlaysFirst(playsFirst);

        setContentsOfTextView(R.id.labelOutput, x.getStatus());

    }
    private void computeButtonMove(View view){
        String textRow = getInputOfTextField(R.id.inputRow);
        String textCol = getInputOfTextField(R.id.inputCol);

        int row = Integer.parseInt(textRow);
        int col = Integer.parseInt(textCol);
